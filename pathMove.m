...
for m = 1:(num_waypoints-1)
    ...
    Estimated_angle = Estimated_Bot.getBotAng();%radian
    turn_angle = angle-Estimated_angle; %radian
    
    if ((turn_angle <pi && turn_angle >0)  || (turn_angle >-pi && turn_angle <0))
        turn_angle = turn_angle;
    elseif turn_angle >pi
    ...
    %if (botScan(1)>= distance*0.5)&&(difference < 2000);
